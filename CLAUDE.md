...
Toda contagem de tela e de passagem é do **plantão dele**, não do serviço. Tela que desenha os 34
leitos e pinta de "vago" o que não está no banco está mentindo: aquele leito não está vago, está
fora do sistema dele.

## Vetado — não propor de novo

| O quê | Situação |
|---|---|
| **SBAR** e qualquer formato de passagem importado de fora | vetado pelo operador |
...
| `src/types/` | `clinical.ts` (à mão) · `supabase.ts` (gerado) |
| `src/styles/globals.css` | o tema inteiro — Tailwind 4 é CSS-first, não existe `tailwind.config.ts` |
| `supabase/migrations/` | migrations. **Não** é retrato do banco vivo |

## Regras que não se negociam

- **Dado clínico ausente é `null`, nunca estimado.** Não inferir lab, sinal vital, dose ou ID que
  não esteja na fonte. É segurança do paciente, não estilo.
...
- **Seta ↑ ↓ é proibida em texto clínico.** A direção vai em palavra: "lactato em queda". Vale para
  código, comentário, doc e texto gerado pelo app.
...
`_material/` e `**/amostras/` têm **dado real de paciente**. Não copiar de lá para `src/` nem para
`tests/` — fixture de teste é sintética.
...
| | |
|---|---|
| 13 tabelas · 8 views · 16 policies | as policies de dono dormentes foram removidas na faxina de 08-ago |
| 15 pacientes · 16 evoluções | |
| `sofa_total` preenchido | **0 de 16** — falta bilirrubina e PaO2/FiO2 a montante, não é falha de código |
| `alert_rules` | 25 regras ativas (já foi reportada como "vazia", e não era) |
