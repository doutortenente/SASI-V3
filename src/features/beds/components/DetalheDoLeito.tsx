...
function BlocoAlertas({ pacienteId, agoraISO }: { pacienteId: string; agoraISO: string }) {
  ...
  return (
    <section aria-label="Alertas abertos">
      ...
      {consulta.isPending && <p className="text-texto-tenue mt-1 text-xs">Consultando o banco…</p>}
      ...
      {consulta.data && consulta.data.length === 0 && (
        <p className="text-texto-tenue mt-1 text-xs">Nenhum alerta aberto.</p>
      )}
      {consulta.data && consulta.data.length > 0 && (
          ...
          {consulta.data.map((a) => (
            <li key={a.id} className="flex items-center gap-2">
              <div className="min-w-0 flex-1">
                <p className="text-texto-corpo text-xs">
                  <span
                    className={`text-2xs mr-1.5 rounded-sm px-1.5 py-0.5 font-semibold tracking-wide uppercase ${CLASSE_SEVERIDADE[a.severidade]}`}
                  >
                    {ROTULO_SEVERIDADE[a.severidade]}
                  </span>
                  {a.mensagem}
                </p>
                <p className="text-texto-tenue text-2xs" data-clinical-number>
  ...
  return (
    <section aria-label="Pendências abertas">
      ...
      {!erro && pendencias === undefined && (
        <p className="text-texto-tenue mt-1 text-xs">Consultando o banco…</p>
      )}
          ...
          {pendencias.map((p) => (
            <li key={p.id} className="flex items-center gap-2">
              <div className="min-w-0 flex-1">
                <p className="text-texto-corpo text-xs">
                  <span
                    className={`text-2xs mr-1.5 rounded-sm px-1.5 py-0.5 font-semibold tracking-wide uppercase ${PRIORIDADE[p.prioridade].classe}`}
                  >
    ...
    <section aria-label="Dispositivos com dias de uso">
      ...
      {dispositivos && dispositivos.length > 0 && (
        <ul className="mt-1 space-y-0.5">
          {dispositivos.map((d) => (
            <li key={d.episodio_id ?? `${d.tipo}-${d.data_inicio}`} className="text-texto-corpo text-xs">
...
export function DetalheDoLeito({
  ...
  return (
    <div className="border-borda-sutil space-y-3 border-t p-3.5 pt-3">
      ...
      <section aria-label="O que mudou">
        ...
        <p className="text-texto-corpo mt-1 text-xs">
