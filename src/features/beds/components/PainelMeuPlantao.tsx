...
function FaixaDeAviso({children}: {children: React.ReactNode}) {
    return (
        <p className="mb-3 rounded-md bg-gravidade-critico-bg px-3 py-2 text-xs font-medium text-gravidade-critico-text">
            {children}
        </p>
    );
}
...
export function PainelMeuPlantao({
    ...
    return (
        <>
            ...
            {pendencias.erro && (
                <FaixaDeAviso>
                    Falha ao ler as pendências ao vivo: os cards mostram o retrato da última carga da página.{' '}
                    {pendencias.erro.message}
                </FaixaDeAviso>
            )}
            {dispositivos.erro && (
                <FaixaDeAviso>
                    Falha ao ler os dispositivos: a lista de dias de uso está SEM DADO.{' '}
                    {dispositivos.erro.message}
                </FaixaDeAviso>
            )}

            {/* Canal ao vivo caído: o dado pode estar velho e a tela tem que dizer. */}
            {(alertas.desatualizado || pendencias.desatualizado) && (
                <p className="mb-3 rounded-md bg-gravidade-watcher-bg px-3 py-2 text-xs font-medium text-gravidade-watcher-text">
                    ...
                    {[
                        ...
                        .filter(Boolean)
                        .join(' e ')}
                    ). Os números podem estar desatualizados até a conexão voltar.
                </p>
            )}
