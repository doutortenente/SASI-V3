...
export interface ResultadoSofa {
    ...
    /**
     * O que falta COLHER — dado ausente que alguém pode ir buscar e fechar o
     * escore. É lista de tarefa.
     *
     * IMPORTANT: não confundir com `suprimidos`. O `sofa.ts` do sasi-import já
     * mantinha `missing[]` e `suppressed[]` como arrays separados, e a fusão dos
     * dois pede à equipe que vá colher uma coisa que não existe: nenhum exame
     * conserta "neurológico suprimido por sedação ativa".
     */
    faltando: string[];
    /**
     * O que foi deliberadamente NÃO pontuado — decisão do motor, não falha de
     * coleta. Hoje só o neurológico sob sedação ativa. Não é acionável por exame:
     * ou a sedação sai, ou alguém registra o Glasgow pré-sedação.
     */
    suprimidos: string[];
    ressalvas: string[];
    /**
     * Instante do cálculo, em ISO 8601 — o `computedAt` do contrato do
     * sasi-import. Fecha a trilha de auditoria junto com `componentes`,
     * `faltando` e `suprimidos`.
     *
     * Por que a data da evolução não basta: a granularidade de tempo do SASI é o
     * plantão (`ts::date`). O SOFA da passagem e o SOFA recalculado à tarde, com
     * a bilirrubina que voltou do laboratório, caem no MESMO dia — dentro de
     * `evolucoes.sofa_snapshot` (jsonb) os dois ficam indistinguíveis sem este
     * campo. Com ele, dá para dizer qual é o mais novo e o que mudou entre eles.
     *
     * `null` quando o relógio recebido é inválido: carimbo não se inventa, pela
     * mesma regra que vale para dado clínico.
     */
 ...
 *
 * `agora` é o relógio, e ele entra pela porta de propósito. O import carimbava
 * `new Date()` DENTRO da função; aqui isso quebraria duas coisas: a regra de
 * cálculo clínico como função pura, e o determinismo do teste — um assert sobre
 * o carimbo nunca fecharia duas vezes seguidas.
 *
 * Nenhum ponto do escore depende deste argumento. Ele só preenche
 * `calculadoEm`. Chamar sem ele usa o relógio do sistema, que é o que quase
 * todo chamador quer; o teste passa uma data fixa e crava o valor.
 */
export function calcularSofa(e: EntradaSofa, agora: Date = new Date()): ResultadoSofa {
    ...
    // Duas causas, duas listas. "Faltou bilirrubina" é tarefa de coleta;
    // "neuro suprimido por sedação" não tem exame que resolva.
    const faltando = chaves
        .filter((k) => componentes[k].pontos === null && componentes[k].suprimido === undefined)
        .map((k) => `${ROTULO[k]}: ${componentes[k].faltou ?? 'sem dado'}`);

    const suprimidos = chaves
        .filter((k) => componentes[k].suprimido !== undefined)
        .map((k) => `${ROTULO[k]}: ${componentes[k].suprimido}`);
    ...
    return {
        ...
        suprimidos,
        ressalvas,
        // Relógio quebrado não vira carimbo chutado — vira null, como todo dado sem fonte.
        calculadoEm: Number.isNaN(agora.getTime()) ? null : agora.toISOString(),
    };
}
