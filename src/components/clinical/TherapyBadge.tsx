...
export function TherapyBadge({
  ...
  const e = ESTILO[tipo];
  const mostraContagem = contagem !== null && contagem !== undefined && Number.isFinite(contagem) && contagem > 0;

  return (
    <span
      title={e.titulo}
      className={`text-2xs inline-flex items-center gap-1 rounded-sm px-1.5 py-0.5 font-semibold tracking-wide ${e.classe} ${pulsar ? 'sasi-critical-pulse' : ''}`}
    >
      {rotulo ?? e.rotulo}
      {mostraContagem && (
