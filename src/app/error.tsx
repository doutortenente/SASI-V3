...
export default function ErroDoWarRoom({error, reset}: {error: Error & {digest?: string}; reset: () => void}) {
    useEffect(() => {
        ...
        <main className="mx-auto max-w-2xl p-6">
            <div className="rounded-xl border-2 border-destructive bg-card p-6">
                <h1 className="text-lg font-semibold text-destructive">
                    Não foi possível ler o painel de leitos
                </h1>

                <p className="mt-3 text-sm">
                    Esta tela está <strong>sem dado para mostrar</strong>. Isto <strong>não</strong> significa
                    que os leitos estão vazios nem que os pacientes estão sem registro — significa que a
                    leitura do banco falhou. Não tire conclusão clínica desta tela.
                </p>
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                    <button
                        type="button"
                        onClick={reset}
                        className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                    >
                        Tentar ler de novo
                    </button>
                </div>
                ...
                <p className="mt-4 text-xs text-muted-foreground">
                    Enquanto isto não voltar, a fonte da verdade é o prontuário e a folha de enfermagem à
                    beira do leito.
                </p>
            </div>
        </main>
